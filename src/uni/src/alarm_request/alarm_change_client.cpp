...
int main(int argc, char **argv)
{
  ...
  std::shared_ptr<rclcpp::AsyncParametersClient> client =
      std::make_shared<rclcpp::AsyncParametersClient>(node, "/alarmserver");
  ...
  auto my_param2 = rclcpp::Parameter("option", atoi(argv[1]));
  parameters.push_back(my_param2);
  auto my_param3 = rclcpp::Parameter("out_time", argv[2]);
