...
class Person():
    ...
    def save_faces(self, base_dir):
        pathname = os.path.join(base_dir, self.name)  # pathname = /base_dir/self.name
        try:
    ...
    # def my_save_faces(self, face):
    #     pathname = os.path.join("result", self.name)
    #     try:
    #         shutil.rmtree(pathname)
    #     except OSError as e:
    #         pass
    ...
    def load(cls, pathname, face_encodings):
        ...
        for face_filename in os.listdir(pathname):
            ...
            if image.size == 0:
                continue
            if face_filename in face_encodings: # known person & recalculate
                face_encoding = face_encodings[face_filename]
            else: # unknown person -> encoding
                #print(pathname, face_filename, "calculate encoding")
                face_encoding = Face.get_encoding(image)
            if face_encoding is None: # encoding error / no face
...
class PersonDB():
    ...
    def save_db(self, dir_name):
        ...
        #self.save_montages(dir_name)  # del
