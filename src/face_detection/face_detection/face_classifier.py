...
from .ros2_bridge import FacePub
...
class FaceClassifier():
    ...
    def detect_faces(self, frame):
        boxes = self.locate_faces(frame) # find faces in frame
        if len(boxes) == 0: # no face
            return []
    ...
    def my_draw_name(self, frame, face):
        print("arg.display")
        color = (255, 0, 0)  # b, g, r
        thickness = 2
        (top, right, bottom, left) = face.location
        
        #print("(" + str((bottom + top) / 2) + "," + str((right + left) / 2) + ")")
...
def main():
    ...
    use_ros = False # True : use ROS2 and you need to publish image from raspberry pi. False : use Webcam to get image

    if use_ros:
        ros2_pub = FacePub()
        ros2_node = ROS2Bridge() #ROS2
        img = ros2_node.get_image()
        ...
        img = cv2.VideoCapture(src_file)

        if not img.isOpened():
            print("cannot open inputfile", src_file)
            exit(1)    

        frame_width = img.get(cv2.CAP_PROP_FRAME_WIDTH)
        frame_height = img.get(cv2.CAP_PROP_FRAME_HEIGHT)
        frame_rate = img.get(5)

    
    frames_between_capture = int(round(frame_rate * args.seconds))

    print("source", args.inputfile)
    print("original: %dx%d, %f frame/sec" % (frame_width, frame_height, frame_rate))
    ratio = float(args.resize_ratio)
    if ratio != 1.0:
        s = "RESIZE_RATIO: " + args.resize_ratio
        s += " -> %dx%d" % (int(frame_width * ratio), int(frame_height * ratio))
        print(s)
    print("process every %d frame" % frames_between_capture)
    print("similarity shreshold:", args.threshold)
    if args.stop > 0:   # stop option
        print("Detecting will be stopped after %d second." % args.stop)

    # load person DB
    #pkg_path = os.path.join(get_package_prefix('face_detection'))
    #result_dir = pkg_path + "/result"
    ...
    num_capture = 0
    if args.capture:    # capture option
    ...
    def signal_handler(sig, frame):
        ...
        running = False
        if use_ros:
            ros2_node.destroy_node() #ROS2
            ros2_pub.destroy_node()
            rclpy.shutdown() #ROS2

    prev_handler = signal.signal(signal.SIGINT, signal_handler)

    if args.display:  # while display
    ...
    while running:
        if use_ros:
            rclpy.spin_once(ros2_node) #ROS2
            img = ros2_node.get_image() #ROS2
            frame = img
        else:
            ret, frame = img.read()
        ...
        face_len_cnt = 0
        face_len_save = None
        faces = fc.detect_faces(frame) # face detection in frame
        for face in faces: #face (filename, image, face_encoding)
            person = fc.compare_with_known_persons(face, pdb.persons) # compare with known person
            if person: # known person
                if face_len_cnt < person.my_face_len() :
                    face_len_cnt = person.my_face_len()
                    face_len_save = face
                continue
            person = fc.compare_with_unknown_faces(face, pdb.unknown.faces) # compare with unknown person
            if person: # person dectection twice
                pdb.persons.append(person)

        if face_len_save is not None:
            faces.remove(face_len_save)

            if use_ros:        
                (top, right, bottom, left) = face_len_save.location
                msg = (right+left)/2
                
                ros2_pub.set_msg(int(msg))
                rclpy.spin_once(ros2_pub) #ROS2

        if args.display or args.capture: # red boxing
            for face in faces:
                fc.draw_name(frame, face)
            if face_len_save is not None:
                fc.my_draw_name(frame, face_len_save)
