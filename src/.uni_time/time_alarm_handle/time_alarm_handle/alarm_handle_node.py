...
#       parameter type : list(4 string, 1(2) int) -> ['bus/subway', 'the time it takes', 'busNo/subwayNo', stationCode, (wayCode(for Subway))]
#           + tip : 'bus' = '2', subway = '1'
...
class AlarmHandleNode(Node):
    ...
    def get_time(self, request, response):
        if request.request == "alarm_set":
            origin = address_to_lalotude(request.home_address)
            destination = address_to_lalotude(request.office_address)
            traffic = transport_info(origin, destination)
            response.alarm_time_response = cal_time('-', request.timeTOwork, ((int)required_time)+traffic[1]) # timeTOwork(00:00) - required_time(min) - traffic[1](min)
            
        else : # transport_req
            origin = address_to_lalotude(request.home_address)
            destination = address_to_lalotude(request.office_address)
            traffic = transport_info(origin, destination)
            response.transport_response = rt_traffic_info(traffic)
    ...
    def rt_traffic_info(traffic): # return min
        stationID = traffic[4]
        if traffic[0] == '2': # BUS
